    ...
    <div className=' top w-screen h-1/3 bg-[#fffffb] sticky top-0 flex justify-between items-center p-5  gap-5 pb-3 z-10' style={{'fontFamily': 'Josefin Sans, sans-serif'}}>
        ...
        <div className=' topCentre flex  lg:ml-4'>
            <ul className=' TopList mt-4 flex justify-between gap-3 lg:gap-6'>
                ...
                <li className=' TopListItems text-xl lg:text-2xl text-gray-900 hover:text-yellow-900 cursor-pointer' onClick={HandleLogout}>{User && "Logout"}</li>
        ...
        {User ? (
            <>
        <Link to='/settings'><img src={PF + User.ProfilePic} alt='...' className=' TopImage h-10 lg:h-12 w-10 lg:w-12 rounded-[50%] object-contain'/></Link>
